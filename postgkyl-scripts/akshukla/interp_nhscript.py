...
for frame in range(start,end_frame):
    ...
    for isim, sim_name in enumerate(sim_names):
        ...
        #Load neutral moment data
        for species in ["H0"]:
            for mom in ["M0", "M2"]:
                mdata = pg.GData('%s-%s_%s_%d.gkyl'%(sim_name, species,mom,neut_frame))
                grid,val = pg.data.GInterpModal(mdata,poly_order=1,basis_type='ms').interpolate(0)
                val = val.squeeze()
                mom_data[species+mom] = val
            mom_data[species+"Temp"] =  (masses[species]/3) * mom_data[species+"M2"]/mom_data[species+"M0"] / eV
    ...
    for i in range(1,4):
        for s, species in enumerate(["H0"]):
            ax[1,1].plot(mom_data_list[i]["Zavg"][xidx], mom_data_list[i][species+"Temp"][xidx,:], color = "tab:red")
    ...
    for i in range(6,9):
        for s, species in enumerate(["H0"]):
            ax[1,0].plot(mom_data_list[i]["Zavg"][xidx], mom_data_list[i][species+"Temp"][xidx,:], color = "tab:red")
    ...
    for i in range(10,12):
        for s, species in enumerate(["elc", "ion"]):
            #ax[0,1].plot(mom_data_list[i]["z"], mom_data_list[i][species+"M0"][xidx_core,:], color = colors[s])
            ax[0,1].plot(raw_mom_data_list[i]["z"], raw_mom_data_list[i][species+"M1"][xidx_core,:], color = colors[s])
    ...
    for i in [2,10]:
        for s, species in enumerate(["H0"]):
            psiax[1,1].plot(mom_data_list[i]["Ravg"][:,zidx[i]], mom_data_list[i][species+"Temp"][:,zidx[i]], color = 'tab:red')
    ...
    for i in [7,11]:
        for s, species in enumerate(["H0"]):
            psiax[1,0].plot(mom_data_list[i]["Ravg"][:,zidx[i]], mom_data_list[i][species+"Temp"][:,zidx[i]], color = 'tab:red')
    ...
    for i in [8, 9]:
        for s, species in enumerate(["elc", "ion"]):
            psiax[0,2].plot(mom_data_list[i]["x"], mom_data_list[i][species+"Q"][:,-1]/1e6*np.sin(mom_data_list[i]["Bratio"]),color = colors[s])
            #psiax[0,2].plot(raw_mom_data_list[i]["x"], raw_mom_data_list[i][species+"Q"][:,-1],color = colors[s])
    ...
    for i in [3, 4]:
        for s, species in enumerate(["elc", "ion"]):
            psiax[0,3].plot(mom_data_list[i]["x"], mom_data_list[i][species+"Q"][:,-1]/1e6*np.sin(mom_data_list[i]["Bratio"]),color = colors[s])
            #psiax[0,3].plot(raw_mom_data_list[i]["x"], raw_mom_data_list[i][species+"Q"][:,-1],color = colors[s])
