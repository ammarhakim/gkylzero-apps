...
# Check against an analytical shift
...
psi_check = np.zeros(len(r))
shift_check = np.zeros(len(r))
shift_check_basic = np.zeros(len(r))
myshift_check = np.zeros(len(r))
for i in range(len(r)):
    psi_check[i] = psi_fi(r[i], -np.pi)
    shift_check[i] = shift_up_angle_i(r[i])
    shift_check_basic[i] = shift_lo_angle_i_basic(r[i])
    myshift_check[i] = alpha(r[i], np.pi-1e-14, 0) - alpha(r[i], -np.pi+1e-14, 0)

psi_min = SIBRY - 0.0004
psi_max = SIBRY + 0.0004

indices = np.where((psi_check > psi_min) & (psi_check < psi_max))[0]
r_check = r[indices]
psi_check = psi_check[indices]
shift_check = shift_check[indices]
shift_check_basic = shift_check_basic[indices]
myshift_check = myshift_check[indices]


vals = pg.GData('../../../gk_ltx_iwl_miller_3x2v_p1-deltats_dir2.gkyl').get_values()
grid = fix_gridvals(pg.GData('../../../gk_ltx_iwl_miller_3x2v_p1-deltats_dir2.gkyl').get_grid()[0])
shift_num = vals[:,1,0,0]/2 # because of the surface basis

plt.figure()
#plt.plot(psi_check, shift_check, label = "Analytical Shift")
plt.plot(psi_check, shift_check_basic, label = "Analytical Shift Basic")
#plt.plot(psi_check, myshift_check, label = "Analytical Shift")
plt.plot(grid, shift_num, label = "Numerical Shift")
plt.xlabel('psi')
plt.legend()
