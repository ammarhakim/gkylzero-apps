...
masses["elc"] = me
masses["ion"] = 2.014*mp
masses["molecule"] = 2.0*2.014*mp

# Set up names for data loading
#sim_dir = './simdata/step14_try1/'
sim_dir = './'
...
for isim, sim_name in enumerate(sim_names):
    ...
    for species in species_list:
        ...
        dist_data[species+"vperp"] = np.sqrt(np.einsum("ij,k->ijk", mom_data["B"],dist_data[species+"mu"])*2/masses[species])
...
def plot_grad(species, cat_data):
    ...
    fig, ax = plt.subplots(1, figsize = (8,3))
    ax.plot(fix_gridvals(xaxis),  cat_data[species + "dfdx"][xind_plot, zind_plot,:,0])
    ax.set_xlabel(xaxis_label, fontsize=20)
    ax.set_ylabel(dist_label + ' at ' + r'$\mu=0$', fontsize=20)
    fig.tight_layout()
...
def plot_species(species, bidx, xind, zind_plot):
    ...
    if zind_plot == zind_mid:
        ...
        if(species in ["ion", "molecule"]):
            phi_trapping_line1 = np.sqrt( (lin_x**2 + 2*eV/masses[species] * (phiplot - phimax)) / ( Bmax/Bplot - 1) )
        elif(species=="elc"):
    ...
    fig, ax = plt.subplots(1, figsize = (8,3))
    ax.plot(fix_gridvals(xaxis), dist_data_list[bidx][species][xind, zind_plot,:,0])
    ax.set_xlabel(xaxis_label, fontsize=20)
    ax.set_ylabel(dist_label + ' at ' + r'$\mu=0$', fontsize=20)
    fig.tight_layout()
...
def sum_distributions_on_common_grid(vpar1, vperp1, f1, vpar2, vperp2, f2):
    """
    Sum two distribution functions defined on different 2D (vpar, vperp) grids.
    Projects f2 onto f1's grid.
    
    Parameters:
    vpar1, vperp1 : 1D arrays defining the grid for species 1 (Target Grid)
    f1            : 2D array of distribution data for species 1 (vpar, vperp)
    vpar2, vperp2 : 1D arrays defining the grid for species 2 (Source Grid)
    f2            : 2D array of distribution data for species 2 (vpar, vperp)
    
    Returns:
    f_total       : Summed distribution (f1 + f2_interpolated) on grid 1.
    """
    
    # 1. Create the interpolator for the second species
    # bounds_error=False and fill_value=0.0 are CRITICAL. 
    # This ensures that if Grid 1 extends beyond Grid 2, we assume f2 is 0.
    ...
    # 2. Create the meshgrid of coordinates for the target grid (Species 1)
    # We need to flatten this to pass into the interpolator
    vpar1_grid, vperp1_grid = np.meshgrid(vpar1, vperp1, indexing='ij')
    
    # Stack them into a list of (vpar, vperp) points
    # Shape becomes (N_points, 2)
    eval_points = np.stack((vpar1_grid.flatten(), vperp1_grid.flatten()), axis=-1)
    
    # 3. Evaluate f2 on Grid 1
    f2_on_grid1_flat = interp_func(eval_points)
    
    # Reshape back to the original 2D shape of Grid 1
    f2_on_grid1 = f2_on_grid1_flat.reshape(vpar1_grid.shape)
    
    # 4. Sum them up
    f_total = f1 + f2_on_grid1
    
    return f_total

# --- Example Usage Context ---
# Assume you load your Gkeyll data here, e.g.:
# data_D = GData("D_plus_file.bp")
# data_D2 = GData("D2_plus_file.bp")
...
# Sum them (Projecting D2+ onto the likely wider D+ grid)
# f_sum = sum_distributions_on_common_grid(vpar_D, vperp_D, f_D, 
#                                          vpar_D2, vperp_D2, f_D2)

# Now you can plot f_sum using vpar_D and vperp_D

def plot_ionmol_sum(bidx, xidx,zidx):
    vpar1 =  fix_gridvals(dist_data_list[bidx]["ionvpar"])
    vperp1 = fix_gridvals(dist_data_list[bidx]["ionvperp"][xidx,zidx])
    f1 = dist_data_list[bidx]["ion"][xidx,zidx]

    vpar2 =  fix_gridvals(dist_data_list[bidx]["moleculevpar"])
    vperp2 = fix_gridvals(dist_data_list[bidx]["moleculevperp"][xidx,zidx])
    f2 = dist_data_list[bidx]["molecule"][xidx,zidx]

    f_total = sum_distributions_on_common_grid(vpar1, vperp1, f1, vpar2, vperp2, f2)

    integrand = f_total * vperp1
    f_reduced = np.trapz(integrand, x=vperp1, axis=1)
    ...
    ax.set_xlabel(xaxis_label, fontsize=20)
    ax.set_ylabel(r'$v_\perp\, [m/s]$', fontsize = 20)
    im = ax.pcolormesh(vpar1, vperp1, f_total.T, cmap='inferno', shading="gouraud")
    dist_label =  r'$f_{D+} + f_{D2+}$'
    divider = make_axes_locatable(ax)
    cax = divider.append_axes('right', size='5%', pad=0.05)
    cbar = fig.colorbar(im, cax=cax, orientation='vertical', label = dist_label)
    cbar.set_label(label = dist_label, fontsize=20)
    ...
    ax.plot(vpar1, f_reduced)
    ax.set_xlabel(r'$v_\parallel\, [m/s]$', fontsize=16)
    ax.set_ylabel(r'$\int dv_\perp v_\perp (f_{D+} + f_{D2+})$', fontsize=16)
    ax.ticklabel_format(axis='x', style='sci', scilimits=(0,0))
    ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
