...
def divertor_plate_func_in(s):
    ...
    """
    s = np.asarray(s)
    RZ_lo = np.array([2.14, -0.675])
    RZ_up = np.array([2.08, -0.645])
    R = (1-s)*RZ_lo[0] + s*RZ_up[0]
    Z = (1-s)*RZ_lo[1] + s*RZ_up[1]
    return R, Z

def divertor_plate_func_out(s):
    """
    Python version of divertor_plate_func_out
    s : float or numpy array in [0, 1]
    returns (R, Z)
    """
    s = np.asarray(s)
    RZ_lo = np.array([2.243, -0.7143])
    RZ_up = np.array([2.3, -0.735])
    R = (1-s)*RZ_lo[0] + s*RZ_up[0]
    Z = (1-s)*RZ_lo[1] + s*RZ_up[1]
    return R, Z
...
def analyze_and_plot(gfile_path, plot_miller, shafranov_param, save_plots, output_dir, figure_format, limiter_segments):
    ...
    psi_axis = gfile_data["simagx"]
    print(f"Psi at magnetic axis: {psi_axis}")
    psi_sep = gfile_data["sibdry"]
    print(f"Psi at separatrix (LCFS): {psi_sep}")
    # Print minor axis
    a_minor = (np.max(gfile_data["rbdry"]) - np.min(gfile_data["rbdry"])) / 2.0
    print(f"Calculated minor radius a: {a_minor} m")
    ...
    svals = np.linspace(0, 1, 20)
    Rout, Zout = divertor_plate_func_out(svals)
    Rin, Zin = divertor_plate_func_in(svals)
    print(Rout, Zout)
