...
def func_data(sp, fr, phiInt):
    spFile=dataDir+simName+'-'+sp+'_BiMaxwellianMoments_%d'%fr+'.gkyl'
    spData = pg.data.GData(spFile)
    spInterp = pg.GInterpModal(spData, polyOrder, basisType)    
    xInt, spDenInt = spInterp.interpolate(0)
    xInt, spTparInt = spInterp.interpolate(2)
    xInt, spTperpInt = spInterp.interpolate(3)
    spTempInt = (spTparInt + 2 * spTperpInt) / 3
...
#for i in [0, 1, 2]:
    ...
    ax1.cla()
    ax1.set_title('Time = %d'%data_num[i]+r' $\mu s$', fontsize=ttlSize)
    ax1.set_xlabel(r'$R - R_{LCFS}$', fontsize=lblSize)
