...
#========== Calculate Mapped Lx ==========
dataDir = ''
outDir = os.path.join(os.getcwd(), 'blobData2')
simName = utils.find_prefix('-field_0.gkyl', '.')
...
blobXwidth = blobData[:,2] / rhos
blobXvel = blobData[:,3] / cs
blobXc = blobData[:,4]
...
plt.hist(blobXvel,bins=nbins,range=[xmin,xmax],color=plt_color,weights=weights)
plt.axvline(x=xvelAve,color=plt_mean,linestyle='--',label=r'mean')
...
fig = plt.figure(figsize=(6.0, 4.5))
plt.hist(blobXwidth,bins=nbins,range=[xmin,xmax],color=plt_color,weights=weights)
plt.axvline(x=sizeAve,color=plt_mean,linestyle='--',label=r'mean')
