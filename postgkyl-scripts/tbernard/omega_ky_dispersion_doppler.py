...
def get_cell_avg_profile(gdata, component_idx, z_slice_idx):
    dg = pg.GInterpModal(gdata, poly_order=1, basis_type='ms')
    raw_coeffs_3d = dg._getRawModal(component_idx)
    cell_avg_3d = raw_coeffs_3d[..., 0] / (2**1.5)
...
def calculate_normalization(elc_prefix, fstart, fend, x_idx, z_idx):
    ...
    for tf in range(fstart, fend + 1):
        try:
            ...
            ne_1d = get_cell_avg_1d(elc_data, 0, x_idx, z_idx)
            Tpar_1d = get_cell_avg_1d(elc_data, 2, x_idx, z_idx)
            Tperp_1d = get_cell_avg_1d(elc_data, 3, x_idx, z_idx)
...
def calculate_background_parameters(file_prefix, fstart, fend, x_idx, z_idx, x_vals):
    ...
    for tf in range(fstart, fend + 1):
        try:
            ...
            profiles['ne'].append(get_cell_avg_profile(elc_data, 0, z_idx))
            Tepar, Teperp = get_cell_avg_profile(elc_data, 2, z_idx), get_cell_avg_profile(elc_data, 3, z_idx)
            profiles['Te'].append((Tepar + 2*Teperp) / 3.0)
            Tipar, Tiperp = get_cell_avg_profile(ion_data, 2, z_idx), get_cell_avg_profile(ion_data, 3, z_idx)
...
def assemble_time_series(file_prefix, file_suffix, component, fstart, fend, x_idx, z_idx):
    ...
    for tf in range(fstart, fend + 1):
        ...
        try:
            ...
            time_series.append(get_cell_avg_1d(gdata, component, x_idx, z_idx))
...
def calculate_doppler_velocity(prefix, fstart, fend, x_idx, z_idx, x_vals):
    """Calculates the time-averaged E_r / B velocity."""
    print("\nCalculating Doppler shift velocity...")
    # Load B field
    try:
        bmag_data = pg.GData(prefix + "-bmag.gkyl")
        # Using cell-averaged B-field for consistency
        b_field_1d = get_cell_avg_1d(bmag_data, 0, x_idx, z_idx)
        B0 = np.mean(b_field_1d) # Use the local, binormal-averaged B
    except Exception as e:
        print(f"Error loading B-field file '{prefix }-bmag.gkyl' : {e}. Cannot calculate Doppler shift.")
        return 0.0
    ...
    dx = x_vals[1] - x_vals[0]
    for tf in range(fstart, fend + 1):
        print(f"Loading phi frame {tf} for E_r...", end='\r')
        try:
            phi_data = pg.GData(f"{prefix}-field_{tf}.gkyl")
            gradx_phi_1d = gradient_1d(phi_data, 0, x_idx, z_idx)
            Vy_series.append(gradx_phi_1d / B0)  # V_E,y = (d(phi)/dx)/B0
        except Exception:
            continue
...
def plot_dispersion(power_spectrum, omega_axis, ky_axis, V_doppler, metadata, theory_params=None):
    ...
    if theory_params:
        ...
        ax.plot(ky_rhos_pos, omega_tem, 'w--', linewidth=2.5, label='TEM / RBM (fluid)')
        ...
        ax.plot(ky_rhos_pos, omega_itg, 'r:', linewidth=2.5, label='ITG (fluid)')
    
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label(r'log$_{10}$ |$F(\omega, k_y)$|$^2$')
    
    ax.set_xlabel(r'$k_y \rho_s$')
    ax.set_ylabel(r'$\omega_{plasma}$ (Plasma Frame Freq. [rad/s])')
    
    qty_name = metadata['quantity']
    ax.set_title(f'Doppler-Corrected Dispersion for ${qty_name}$ at x_idx={metadata["x_idx"]}')
...
def main():
    ...
    V_doppler = calculate_doppler_velocity(file_prefix, args.fstart, args.fend, x_idx, z_idx, x_vals)
    ...
    plot_dispersion(power_spectrum, omega_axis, ky_axis, V_doppler, metadata, theory_params)
