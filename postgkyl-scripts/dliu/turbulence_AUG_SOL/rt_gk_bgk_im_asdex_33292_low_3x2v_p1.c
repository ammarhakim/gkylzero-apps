...
void eval_density(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double profile = (-2.35*tanh(62.0*(rho-1.0)) + 2.712) * 1.0e18;

  // Multiply by a function that smoothly drops the temperature by 5X towards
  // the divertor plates.
  //profile *= 0.2+0.8*exp(-0.25*pow(z/(M_PI/1.5),8));
...
void eval_temp_elc(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double profile = (43.5191 * exp(-119.6376*(rho-1.0)) + 8.9418) * eV;
...
void eval_temp_ion(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double profile = (1871814.04106884*pow(rho, 3.0) - 5713049.12690327*pow(rho, 2.0) + 5807837.75249853*rho - 1966419.18188765 ) * eV;
...
struct gk_app_ctx create_ctx(void)
{
  ...
  char eqdsk_file[128] = "./gyrokinetic/data/eqdsk/asdex_33292_1.800.eqdsk";
  ...
  double Te_floor = 5.0*eV; // Min Te in IC.
  double Ti_floor = 5.0*eV; // Min Ti in IC.
  ...
  double x_min = 0.080059;
  double x_max = 0.097067;
  ...
  // Source parameters
  int num_sources = 2;
  double P_exp = 0.235e6; // P_sol measured [W]
...
//    },
  ...
  struct gkyl_gk app_inp = {
    .name = "gk_bgk_im_asdex_33292_low_3x2v_p1",
