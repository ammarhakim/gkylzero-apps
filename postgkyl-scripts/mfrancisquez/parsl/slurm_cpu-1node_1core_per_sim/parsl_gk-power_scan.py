...
#[
#[ This runs each simulation on a CPU within a Slurm job.
#[
#[ Manaure Francisquez.
#[ November 2025.
#[
  ...
  executors=[
    HighThroughputExecutor(
      ...
      provider=LocalProvider(
        ...
        launcher=SrunLauncher(overrides='-c 1 --cpu_bind=cores'),
...
def gk_sim(stdout=(gk_exec_name+".out", "w"), stderr=(gk_exec_name+".err", "w"),
           sim_dir='./', exec_full_path = './'+gk_exec_name, power=4.5e6):
  #[ Go to directory for this sim.
  import os
  os.chdir(sim_dir)

  #[ Command to run Gkeyll (as a string).
  gk_cmd = exec_full_path + " -o Pin=" + str(power)
  return gk_cmd
