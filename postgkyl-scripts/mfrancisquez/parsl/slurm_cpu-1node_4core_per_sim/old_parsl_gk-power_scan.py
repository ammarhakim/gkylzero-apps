...
input_power = [3.5e6+i*0.5e6 for i in range(4)]
...
#      cores_per_worker = cores_per_sim,
      cores_per_worker = 1,
#      cpu_affinity = 'block',
      provider = LocalProvider(
        nodes_per_block = num_nodes,
#        nodes_per_block = num_sims,
#        nodes_per_block = int(cores_per_node/cores_per_sim),
        launcher = SrunMPILauncher(overrides='-c '+str(cores_per_sim)+' --cpu_bind=cores'),
        init_blocks = 1,
        max_blocks = 1,
      ),
    )
...
def gk_sim(stdout=(gk_exec_name+".out", "w"), stderr=(gk_exec_name+".err", "w"),
  ...
  gk_cmd = exec_full_path + " -M -d " + str(mpi_processes) + " -o Pin=" + str(power)
  return gk_cmd
